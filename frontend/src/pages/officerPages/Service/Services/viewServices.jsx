...
const viewServices = () => {
    ...
    const fetchServicesAndCategories = async () => {
      try {
        const [serviceRes, categoryRes] = await Promise.all([
          fetch("http://localhost:5000/admin/get-rolewise-service", {
        ...
        const merged = (serviceData?.services || []).map((service) => {
  ...
  const filteredItems = (services || [])
    ...
    .map((item, index) => ({
      ...
      editPath: `/officer/edit-services/${item._id}`,
    }));
